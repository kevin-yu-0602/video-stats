...
do
  ...
  if [[ "$directorPart" == "無導演" ]] || [[ "$directorPart" == "无导演" ]] || [[ "$directorPartMinusOne" == "無導演" ]] || [[ "$directorPartMinusOne" == "无导演" ]]; then
    matchingDirector=$(echo "$filmPart" | perl -C63 -ne "print if s/(?<year>[0-9]{4}) *(?<chName>[\p{Han}\p{Punct}A-Z0-9]*[\p{Han}\p{Punct}]+)? *(?<engName>(?:(?"'!'" [0-9]\.[0-9])[^\/\p{Han}])+)? *(?<country>[\p{Han}]+)? *(?<rating>[0-9]\.[0-9])? *(?<at>\@\w+)? *(?<b>B\d{1,3})? *(?<c>C\d{1,3})? *(?<g>G\d{1,3})? *(?<l>L\d{1,3})? *(?<e>E\d{1,3})? *(?<s>S\d{1,3})? *(?<h>H\d{1,3})? *(?<j>J\d{1,3})? *(?<notes>.*)$/\"$+{country}\",\"\",\"\",\"\",\"\"/")
    matchingFilm=$(echo "$filmPart" | perl -C63 -ne "print if s/(?<year>[0-9]{4}) *(?<chName>[\p{Han}\p{Punct}A-Z0-9]*[\p{Han}\p{Punct}]+)? *(?<engName>(?:(?"'!'" [0-9]\.[0-9])[^\/\p{Han}])+)? *(?<country>[\p{Han}]+)? *(?<rating>[0-9]\.[0-9])? *(?<at>\@\w+)? *(?<b>B\d{1,3})? *(?<c>C\d{1,3})? *(?<g>G\d{1,3})? *(?<l>L\d{1,3})? *(?<e>E\d{1,3})? *(?<s>S\d{1,3})? *(?<h>H\d{1,3})? *(?<j>J\d{1,3})? *(?<notes>.*)$/\"$+{year}\",\"$+{chName}\",\"$+{engName}\",\"$+{rating}\",\"$+{at}\",\"$+{b}\",\"$+{c}\",\"$+{g}\",\"$+{l}\",\"$+{e}\",\"$+{s}\",\"$+{h}\",\"$+{j}\",\"$+{notes}\"/")
  else
    ...
    matchingFilm=$(echo "$filmPart" | perl -C63 -ne "print if s/(?<year>[0-9]{4}) *(?<chName>[\p{Han}\p{Punct}A-Z0-9]*[\p{Han}\p{Punct}]+)? *(?<engName>(?:(?"'!'" [0-9]\.[0-9])[^\/\p{Han}])+)? *(?<rating>[0-9]\.[0-9])? *(?<at>\@\w+)? *(?<b>B\d{1,3})? *(?<c>C\d{1,3})? *(?<g>G\d{1,3})? *(?<l>L\d{1,3})? *(?<e>E\d{1,3})? *(?<s>S\d{1,3})? *(?<h>H\d{1,3})? *(?<j>J\d{1,3})? *(?<notes>.*)$/\"$+{year}\",\"$+{chName}\",\"$+{engName}\",\"$+{rating}\",\"$+{at}\",\"$+{b}\",\"$+{c}\",\"$+{g}\",\"$+{l}\",\"$+{e}\",\"$+{s}\",\"$+{h}\",\"$+{j}\",\"$+{notes}\"/")
  fi
