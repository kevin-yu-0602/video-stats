...
do
  ...
  if [[ "$directorPart" == "無導演" ]] || [[ "$directorPart" == "无导演" ]] || [[ "$directorPartMinusOne" == "無導演" ]] || [[ "$directorPartMinusOne" == "无导演" ]]; then
    ...
    matchingFilm=$(echo "$filmPart" | perl -C63 -ne "print if s/(?<year>[0-9]{4}) *(?<chName>[\p{Han}\p{Punct}A-Z0-9]*\p{Han}[\p{Han}\p{Punct}A-Z0-9]*)? *(?<engName>(?:(?"'!'" [0-9]\.[0-9])[^\/\p{Han}])+)? *(?<rating>[0-9]\.[0-9])? *(?<at>\@[^\s]+)? *(?<b>B[^\s]{1,5})? *(?<c>C[^\s]{1,5})? *(?<g>G[^\s]{1,5})? *(?<l>L[^\s]{1,5})? *(?<e>E[^\s]{1,5})? *(?<s>S[^\s]{1,5})? *(?<h>H[^\s]{1,5})? *(?<j>J[0-9]{1,5})? *(?<notes>.*)$/\"$+{year}\",\"$+{chName}\",\"$+{engName}\",\"$+{rating}\",\"$+{at}\",\"$+{b}\",\"$+{c}\",\"$+{g}\",\"$+{l}\",\"$+{e}\",\"$+{s}\",\"$+{h}\",\"$+{j}\",\"$+{notes}\"/")
  fi
